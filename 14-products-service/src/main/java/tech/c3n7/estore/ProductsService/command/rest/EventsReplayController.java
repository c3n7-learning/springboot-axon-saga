...
public class EventsReplayController {
    ...
    public ResponseEntity<String> replayEvents(@PathVariable String processorName) {
        ...
        if(trackingEventProcessorOptional.isPresent()) {
            TrackingEventProcessor eventProcessor = trackingEventProcessorOptional.get();
            ...
            return ResponseEntity.badRequest().body(
                    String.format("The event processor with a name {%s} is not a tracking event processor.", processorName)
            );
        }
    }
}
